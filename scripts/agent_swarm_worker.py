...
class SwarmWorkerClient:
    ...
    def fetch_state(self) -> dict:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "GKWA-Agent-Swarm-Worker/1.0"})
    ...
    def send_action(self, payload: dict) -> dict:
        ...
        req = urllib.request.Request(url, data=data, headers={"Content-Type": "application/json", "User-Agent": "GKWA-Agent-Swarm-Worker/1.0"})
        try:
            with urllib.request.urlopen(req, timeout=5) as response:
                return json.loads(response.read().decode("utf-8"))
